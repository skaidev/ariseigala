    ...
    <header className="">
      <div className="container">
        <nav className="navbar border-bottom  border-3" role="navigation">
          ...
          <button className="btn d-block d-md-none  border border-white" onClick={() => setMinMenu((c) => !c)} role="button" aria-label="open menu">
        ...
        {categories?.map((cat, i) => (
          <li className="nav-item" key={i}>
            <Link href={`/articles/${cat.slug}`}>
              <a className={`nav-link ${category === cat.slug ? "active" : ""}`} id={cat.slug}>
                {cat.name}
              </a>
            </Link>
          </li>
